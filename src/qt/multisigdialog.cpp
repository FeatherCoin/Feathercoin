...
void MultiSigDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model, SIGNAL(notifyWatchonlyChanged(bool)), this,SLOT(updateAddressBalance()));
    }
}
...
//----------------------
    ...
    if ( isTxCreate )
    {
        ...
        for (unsigned int i = 0; i < mergedTx.vin.size(); i++)
        {
            ...
            //scriptPubKey = false ,prevPubKey =false
...
void MultiSigDialog::checkRawTransaction()
{
    ...
    if ( isTxCreate )
    {
        ...
        CTransaction txv(*rawTx);
        // Sign what we can:
        const CTxIn& txin = rawTx->vin[nCurrentIn];
        CCoins coins;
