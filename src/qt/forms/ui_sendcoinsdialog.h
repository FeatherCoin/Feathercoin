...
public:
    ...
    void setupUi(QDialog *SendCoinsDialog)
    {
        ...
        hashButton = new QPushButton(SendCoinsDialog);
        hashButton->setObjectName(QStringLiteral("hashButton"));
        QIcon icon4;
        icon4.addFile(QStringLiteral(":/icons/transaction_conflicted"), QSize(), QIcon::Normal, QIcon::Off);
        hashButton->setIcon(icon4);

        horizontalLayout->addWidget(hashButton);
    ...
    void retranslateUi(QDialog *SendCoinsDialog)
    {
...
#ifndef QT_NO_TOOLTIP
        radioCustomAtLeast->setToolTip(QApplication::translate("SendCoinsDialog", "If the custom fee is set to 1000 satoshis and the transaction is only 250 bytes, then \"per kilobyte\" only pays 250 satoshis in fee, while \"total at least\" pays 1000 satoshis. For transactions bigger than a kilobyte both pay by kilobyte.", 0));
#endif // QT_NO_TOOLTIP
        radioCustomAtLeast->setText(QApplication::translate("SendCoinsDialog", "total at least", 0));
#ifndef QT_NO_TOOLTIP
        checkBoxMinimumFee->setToolTip(QApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for feathercoin transactions than the network can process.", 0));
#endif // QT_NO_TOOLTIP
        checkBoxMinimumFee->setText(QString());
#ifndef QT_NO_TOOLTIP
        labelMinFeeWarning->setToolTip(QApplication::translate("SendCoinsDialog", "Paying only the minimum fee is just fine as long as there is less transaction volume than space in the blocks. But be aware that this can end up in a never confirming transaction once there is more demand for feathercoin transactions than the network can process.", 0));
#endif // QT_NO_TOOLTIP
        ...
        sendQRButton->setText(QApplication::translate("SendCoinsDialog", "sendQRButton", 0));
        hashButton->setText(QApplication::translate("SendCoinsDialog", "get TxHash", 0));
