...
Value getaccount(const Array& params, bool fHelp)
{
    ...
    string strAccount;
    std::map<CTxDestination, string>::iterator mi = pwalletMain->mapAddressBook.find(address.Get());
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <feathercoinaddress> [minconf=1]\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <tofeathercoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    if (fByAccounts)
    {
        for (std::map<string, tallyitem>::iterator it = mapAccountTally.begin(); it != mapAccountTally.end(); ++it)
        {
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    std::map<string, int64> mapAccountBalances;
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Return information about <feathercoinaddress>.");
