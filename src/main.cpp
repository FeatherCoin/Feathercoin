                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee)
{
    ...
    string reason;
             // if (fDebug) if (fDebug) LogPrintf("AcceptToMemoryPool : nonstandard transaction: %s");
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew) {
    ...
    if (!ReadBlockFromDisk(block, pindexNew))
        return state.Abort(_("Connect a new block to chainActive,Failed to read block."));
    // Apply the block atomically to the chain state.
    int64_t nStart = GetTimeMicros();
    {
        CCoinsViewCache view(*pcoinsTip, true);
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    // Find the fork (From pindexOldTip Retreated pindexNew=pfork) 
...
bool VerifyDB(int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        if (!ReadBlockFromDisk(block, pindex))
            return error("check level 0: VerifyDB() : *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        // check level 1: verify block validity
        if (nCheckLevel >= 1 && !CheckBlock(block, state, pindex->nHeight))
            return error("check level 1: VerifyDB() : *** found bad block at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            CBlock block;
            if (!ReadBlockFromDisk(block, pindex))
                return error("check level 4: VerifyDB() : *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
...
{
    //if (fDebug) LogPrintf("ProcessMessages(%u messages)\n", pfrom->vRecvMsg.size());
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        //if (fDebug) LogPrintf("ProcessMessages(message %u msgsz, %u bytes, complete:%s)\n", msg.hdr.nMessageSize, msg.vRecv.size(), msg.complete() ? "Y" : "N");
