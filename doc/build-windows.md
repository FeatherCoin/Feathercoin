...
Below are some notes on how to build Feathercoin Core for Windows.

The options known to work for building Feathercoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Feathercoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Feathercoin Core applications.
...
Note that for WSL the Feathercoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/Feathercoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
