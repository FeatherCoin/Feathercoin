...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        spends1_id = [self.nodes[0].sendrawtransaction(hexstring=tx, maxfeerate=0) for tx in spends1_raw]

        blocks = []
        blocks.extend(self.nodes[0].generate(1))

        spends2_raw = [create_raw_transaction(self.nodes[0], txid, node0_address, amount=49.98) for txid in spends1_id]
        spends2_id = [self.nodes[0].sendrawtransaction(hexstring=tx, maxfeerate=0) for tx in spends2_raw]
